...
def check_advancement(stage: CurriculumStage, metric_window: list[dict]) -> tuple[bool, str]:
    ...
    # BUGFIX (2026-06-12): also skip NaN/inf. RLlib reports NaN episode metrics
    # for any iteration that completes zero episodes (common with long episodes
    # + small batch). NaN is an instance of float, so it used to pass the filter;
    # then `nan > thr` and `nan < thr` are BOTH False, so the gate falsely
    # returned True and the stage advanced without meeting any criterion.
    averages: dict[str, float] = {}
    for key in stage.advance_conditions:
        metric_key = _condition_key_to_metric(key)
        values = [
            value
            for value in (m.get(metric_key) for m in metric_window)
