...
def _extract_policy_weights(algorithm, policy_id: str = "default_policy"):
    # BUGFIX (2026-07-04, platform bug #7b): previously this read the LOCAL env
    # runner's module (via _get_rl_module -> Algorithm.get_module), which on the
    # new API stack is an inference_only copy -- the saved bundle contained only
    # the actor weights (7 tensors) and silently dropped the critic. Every run
    # seeded from such a bundle restarted with a fresh value function. Prefer the
    # learner group (full module incl. critic); fall back to the old paths.
    learner_group = getattr(algorithm, "learner_group", None)
    if learner_group is not None:
        try:
            weights = learner_group.get_weights(module_ids=[policy_id])
...
def _apply_policy_weights(
    ...
    # BUGFIX (2026-07-04, platform bug #7): the original implementation applied
    # the bundle ONLY to the local env runner (`algorithm.env_runner`) and
    # returned True. On the new API stack with num_env_runners > 0 that made
    # --init-bundle a silent NO-OP: the 4 remote env runners sampled with fresh
    # random weights from iteration 0, and the learner (also fresh) broadcast
    # its own weights to every runner after the first update, discarding the
    # bundle entirely. Every "seeded" run actually trained from scratch.
    # Correct order: (1) load into the learner group (source of truth),
    # (2) push learner weights to ALL env runners (remote + local),
    # (3) keep the old direct local-runner set + old-stack fallbacks.
    applied = False

    learner_group = getattr(algorithm, "learner_group", None)
    if learner_group is not None:
        try:
            # Partial state: only the RLModule weights; optimizer state and
            # missing keys (e.g. critic, when loading an actor-only bundle)
            # are left at their current values (torch load_state_dict
            # strict=False semantics inside RLModule.set_state).
            learner_group.set_state(
                {"learner": {"rl_module": {policy_id: weights}}}
            )
            applied = True
        except Exception:
            applied = False
