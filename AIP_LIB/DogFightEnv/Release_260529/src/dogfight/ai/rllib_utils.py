...
class _LocalRLModuleEnvRunner:
    """Small adapter exposing the EnvRunner state API used by RLActionProvider."""

    def __init__(self, module):
        self.module = module

    def set_state(self, state: dict[str, Any]) -> None:
        weights = state.get("rl_module", state)
        self.module.set_state(weights)


class _LocalRLModuleAlgorithm:
    """Inference-only facade around a single RLlib RLModule.

    RLlib Algorithm construction is heavyweight and can hang in constrained
    desktop inference contexts. The provider only needs get_module(), stop(),
    and EnvRunner.set_state(), so keep the public surface intentionally tiny.
    """

    def __init__(self, module):
        self._module = module
        self.env_runner = _LocalRLModuleEnvRunner(module)
...
def _build_ppo_module_algorithm_from_bundle(metadata: dict):
    import numpy as np
    try:
    ...
    config = PPOConfig()
    config, restored_model_config = _apply_bundle_model_config(
        config,
        metadata.get("metadata", {}),
    )
    if not restored_model_config:
        # Preserve the historical default architecture if an older bundle has no
        # explicit metadata. It matches the team PPO bundles exported here.
        config = _apply_mlp_model_config(
            config,
            {
                "model_config": {
                    "enabled": True,
    ...
    env_config = metadata.get("algorithm_config", {}).get("env_config", {}) or {}
    obs_size = int(env_config.get("observation_size", 26))
    ...
    return _LocalRLModuleAlgorithm(spec.build())


def _build_full_algorithm_from_bundle(metadata: dict):
    try:
        import ray
...
def build_algorithm_from_bundle(metadata: dict):
    """Build an inference backend for a lightweight RLlib bundle.

    The default path constructs only RLlib's RLModule for fast, deterministic
    inference. Set DOGFIGHT_RLLIB_FULL_ALGORITHM_RESTORE=1 to use the legacy
    full Algorithm builder for debugging or training-runtime parity checks.
    """
    if os.environ.get("DOGFIGHT_RLLIB_FULL_ALGORITHM_RESTORE") == "1":
        return _build_full_algorithm_from_bundle(metadata)

    algorithm_class = metadata.get("algorithm_class", "").strip()
    if algorithm_class == "PPO":
        return _build_ppo_module_algorithm_from_bundle(metadata)
    return _build_full_algorithm_from_bundle(metadata)
