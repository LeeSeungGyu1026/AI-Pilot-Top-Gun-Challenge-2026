...
class DogFightEnv(gym.Env):
    ...
    def __init__(
        ...
        self._lufbery_guard = self.config.get("lufbery_guard", {})
        self._timeout_guard = self.config.get("timeout_guard", {})
    ...
    def step(self, action) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if not terminated and not truncated:
            lufbery_triggered = self._update_lufbery_guard()
            if lufbery_triggered and self._lufbery_guard.get("terminate", True):
                terminated = True
                end_condition = "lufbery guard"
                lufbery_penalty = float(self._lufbery_guard.get("penalty", -30.0))
        timeout_no_wez = False
        timeout_penalty = 0.0
        if truncated and self._timeout_guard.get("enabled", False):
            min_wez_steps = int(self._timeout_guard.get("min_wez_steps", 1))
            wez_steps_with_current = self._ep_wez_steps + int(bool(self._in_wez))
            if wez_steps_with_current < min_wez_steps:
                timeout_no_wez = True
                end_condition = "timeout no wez"
                timeout_penalty = float(self._timeout_guard.get("penalty", -30.0))
        ...
        if lufbery_penalty:
            reward = float(reward) + lufbery_penalty
            components = {**components, "lufbery_guard": lufbery_penalty}
        if timeout_penalty:
            reward = float(reward) + timeout_penalty
            components = {**components, "timeout_no_wez": timeout_penalty}
        ...
        self.info = {
            ...
            "ep_lufbery_steps": self._lufbery_steps,
            "timeout_no_wez": timeout_no_wez,
    ...
    def _update_lufbery_guard(self) -> bool:
        cfg = self._lufbery_guard or {}
        if not cfg.get("enabled", False):
            return False

        band = cfg.get("distance_band_m", [900.0, 2600.0])
        lo = float(band[0])
        hi = float(band[1])
        min_ata = float(cfg.get("min_ata_deg", 25.0))
        min_steps = int(cfg.get("min_steps", 600))

        dist = float(self._geo_info._get_distance(self._ownship_state, self._target_state))
        own_ata = abs(float(self._geo_info._get_antenna_train_angle(
            self._ownship_state, self._target_state, True
        )))
        target_ata = abs(float(self._geo_info._get_antenna_train_angle(
            self._target_state, self._ownship_state, True
        )))
        stuck = lo <= dist <= hi and own_ata >= min_ata and target_ata >= min_ata and not self._in_wez
        if stuck:
            self._lufbery_steps += 1
        else:
            self._lufbery_steps = 0
        return self._lufbery_steps >= min_steps
    ...
    def _classify_outcome(
        ...
        if terminated:
            ...
            if end_condition in ("ownship altitude below min", "FDM Update Fail"):
                return "crash"
            if end_condition in ("two circle headon guard fail", "lufbery guard"):
                return "loss"
            return "draw"
        if truncated:
            if end_condition == "timeout no wez":
                return "loss"
            return "timeout"
        return "ongoing"
