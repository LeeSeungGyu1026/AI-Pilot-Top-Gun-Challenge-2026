...
def _extract_custom_metrics(result: dict) -> dict:
    # BUGFIX (2026-06-12): Ray 2.54 new API stack reports the DogFightCallbacks
    # custom metrics under their bare names (e.g. "crash"), not "<name>_mean".
    # The original lookup only tried "<name>_mean", so every stage-advancement
    # metric stayed "n/a" and advance_conditions could never fire (stages only
    # ended at max_iterations). Mirrors train_rllib._extract_custom_metrics.
    cm = result.get("env_runners", {}).get("custom_metrics", {})

    def metric(name: str, default="n/a"):
